...
def test_linear_functional(dim_x, seed, method, sqrt):
    ...
    x, chol_x, _, _, chol_q, _, _ = get_system(dim_x, dim_x)
...
def test_linear_conditional(dim_x, dim_q, seed, method, sqrt):
    ...
    x, chol_x, F, q, chol_q, _, _ = get_system(dim_x, dim_q)
...
def test_sqrt_vs_std(dim_x, seed, method, test_fun):
    ...
    sqrt_function_model = FunctionalModel(test_fun, chol_x)
...
def test_sqrt_vs_std_conditional_transition(dim_x, seed, method):
    ...
    x, chol_x, _, _, _, _, _ = get_system(dim_x, dim_x)
    ...
    sqrt_F_x, chol_Q_lin, sqrt_remainder = method(sqrt_moments_model, chol_x)
    F_x, Q_lin, remainder = method(moments_model, x)
...
def test_get_conditional_model(dim_x, dim_q, seed, method, sqrt):
    ...
    x, chol_x, _, q, chol_q, _, _ = get_system(dim_x, dim_x)
    ...
    if dim_x != dim_q:
        with pytest.raises(NotImplementedError):
            _ = get_conditional_model(f, q, method)
        return

    moments_model = get_conditional_model(f, q, method)
